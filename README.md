...
To ensure IPFabric is automatically re-installed during future upgrades, create a file named `local_requirements.txt` (if not already existing) in the Nautobot root directory (alongside `requirements.txt`) and list the `ipfabric` package:

```no-highlight
# echo ipfabric >> local_requirements.txt
```
...
PLUGINS = ["nautobot_chatops", "ipfabric"]
