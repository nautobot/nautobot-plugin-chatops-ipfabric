...
# Variables may be overwritten in invoke.yml or by the environment variables INVOKE_IPFABRIC_xxx
namespace = Collection("ipfabric")
namespace.configure(
    {
        "ipfabric": {
            "nautobot_ver": "latest",
...
def docker_compose(context, command, **kwargs):
    ...
    """
    build_env = {"NAUTOBOT_VER": context.ipfabric.nautobot_ver, "PYTHON_VER": context.ipfabric.python_ver}
    compose_command = f'docker-compose --project-name {context.ipfabric.project_name} --project-directory "{context.ipfabric.compose_dir}"'
    for compose_file in context.ipfabric.compose_files:
        compose_file_path = os.path.join(context.ipfabric.compose_dir, compose_file)
...
def run_command(context, command, **kwargs):
    ...
    if is_truthy(context.ipfabric.local):
...
def build(context, force_rm=False, cache=True):
    ...
    print(f"Building Nautobot with Python {context.ipfabric.python_ver}...")
...
def makemigrations(context, name=""):
    ...
    command = "nautobot-server makemigrations ipfabric"
...
def tests(context, failfast=False):
    ...
    if not is_truthy(context.ipfabric.local):
